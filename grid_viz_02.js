...
//  cgrp.classList.add("clickable");
  const upbtn = path("M 120 50 h 5 l -2.5 -2.5 l -2.5 2.5", "#009688", 1, 0.2, "#29B6F6", 1);
  upbtn.classList.add("clickable");
  upbtn.onclick = () => move(model, direction.UP);
  const leftbtn = path("M 118 52 v 5 l -2.5 -2.5 l 2.5 -2.5", "#009688", 1, 0.2, "#29B6F6", 1);
  leftbtn.classList.add("clickable");
  leftbtn.onclick = () => move(model, direction.LEFT);
  const rightbtn = path("M 127 52 v 5 l 2.5 -2.5 l -2.5 -2.5", "#009688", 1, 0.2, "#29B6F6", 1);
  rightbtn.classList.add("clickable");
  rightbtn.onclick = () => move(model, direction.RIGHT);
  const downbtn = path("M 120 59 h 5 l -2.5 2.5 l -2.5 -2.5", "#009688", 1, 0.2, "#29B6F6", 1);
  downbtn.classList.add("clickable");
  downbtn.onclick = () => move(model, direction.DOWN);
...
const draw_axes = (t, c, agent_loc) => {
  ...
  const y_origin = c.y + (c.y_gap * (c.ny - 1));
  const x_axis_y = y_origin + 10;
  const x_axis = new Array(c.nx).fill(x_axis_y).map((y, i) => { return { x: c.x +  (i * c.x_gap), y: y }});
  const y_axis = new Array(c.ny).fill(x_origin).map((x, i) => { return { x: x, y: y_origin - (i * c.y_gap) }});
...
const show_distance = (agent_loc, dest_loc) => {
  ...
  const md_text = text(100, 20, "Manhattan Distance: " + manhattan_dist);
  md_text.id = id;
...
const draw_agent = (t, config, p, l) => {
  ...
  const agent_path_trace = path( p.reduce((acc, dn) => acc + unit_vector_view(dn, config.x_gap, config.y_gap), 
                                 "M" + " " + config.x + " " + (config.y + config.y_gap * (config.ny - 1))), "#FFEE58", 2, 0.7);
...
const to_grid_coords = (model_coords, config) => {
  ...
  return { x: config.x + model_coords.x * config.x_gap,
           y: config.y + (config.ny - 1 - model_coords.y ) * config.y_gap
         };
};
const draw = (model) => {
  const target = document.getElementById("view");
